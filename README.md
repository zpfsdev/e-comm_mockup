...
The repository is organized into a monorepo containing the core application source code. Documentation and utility scripts are excluded from the main branch to maintain a clean source-only environment.
...
> [!NOTE]
> Additional documentation and build scripts are available locally in `docs/` and `scripts/` but are not tracked in the `main` branch.
